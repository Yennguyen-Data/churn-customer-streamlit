...
df_model_comparison = pd.read_csv('data/model_compare_full.csv')
df_model_comparison_10f = pd.read_csv('data/model_compare_10f.csv')

report_xgb = pd.read_csv('data/report_model_xgb.csv')
...
if choice == 'Data Insights':
    ...
    with tab7:
        ...
        st.image('figure/general_dashboard.png')
        st.divider()
...
### 3️⃣ Cost vs. Loyalty Correlation
...
if choice == "Build Model":
    ...
    with tab_choose_model:
        ...
        st.write('''
                ...
                ''')
        st.image('figure/tab_3_feature_important.png')
    ...
    with tab_Build_Model:
        ...
        st.subheader("1️⃣ Modeling Overview")
        st.write("""
        ...
        st.subheader("3️⃣ Handling Class Imbalance")
        ...
        st.subheader("3️⃣ Top 10 Feature Importance")
...
""", language='python') 
        ...
        st.image('figure/confusion_matrix.png')
        st.image('figure/roc_auc.png')

    
        
        st.subheader("5️⃣ Threshold Tuning (Precision / Recall / F1)")
        st.image('figure/threshold.png')
        st.write("""
...
if choice == 'New Customer Prediction':
    ...
    if input_method == "Manual Input":
        ...
        st.write("Fill in the customer features:")

        with st.form("manual_input_form"):
            # Numeric features
            tenure = st.number_input("Tenure (months)", min_value=0, max_value=100, value=12, step=1)
            MonthlyCharges = st.number_input("Monthly Charges ($)", min_value=0.0, max_value=1000.0, value=50.0, step=0.5)
            TotalCharges = st.number_input("Total Charges ($)", min_value=0.0, max_value=10000.0, value=600.0, step=1.0)
            ...
            if submitted:
                ...
                pred_label = predict_pipeline.predict(input_df)[0]
                ...
                st.write(f"**Predicted Class:** {'Churn' if pred_label==1 else 'No Churn'}")
                if pred_label == 1:
                    st.warning("⚠️ Customer is at risk of churning. Consider retention strategies!")
                else:
    ...
    # -------------------- UPLOAD CSV --------------------
        ...
        st.write(", ".join(base_features + numeric_cols))
        ...
        if uploaded_csv is not None:
            ...
            if st.button("Predict Churn for Uploaded CSV"):
                # Predict probability
